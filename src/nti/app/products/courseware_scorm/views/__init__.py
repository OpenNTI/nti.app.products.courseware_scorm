#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
.. $Id$
"""

from __future__ import division
from __future__ import print_function
from __future__ import absolute_import

from nti.app.products.courseware_scorm import MessageFactory

DELETE_ALL_REGISTRATIONS_VIEW_NAME = u'DeleteAllRegistrations'
GET_REGISTRATION_LIST_VIEW_NAME = u'RegistrationList'
GET_SCORM_ARCHIVE_VIEW_NAME = u'SCORMArchive'
SCORM_PROGRESS_VIEW_NAME = u'SCORMProgress'
SYNC_REGISTRATION_REPORT_VIEW_NAME = u'SyncRegistrationReport'

UPDATE_SCORM_VIEW_NAME = u'UpdateSCORM'
CREATE_SCORM_COURSE_VIEW_NAME = u'CreateSCORM'
IMPORT_SCORM_COURSE_VIEW_NAME = u'ImportSCORM'
LAUNCH_SCORM_COURSE_VIEW_NAME = u'LaunchSCORM'
PREVIEW_SCORM_COURSE_VIEW_NAME = u'PreviewSCORM'
REGISTRATION_RESULT_POSTBACK_VIEW_NAME = u'SCORMResultPostback'
SCORM_CONTENT_ASYNC_UPLOAD_UPDATE_VIEW = u'SCORMContentAsyncUploadStatusUpdate'
